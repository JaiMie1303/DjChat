...
const retryFrequency = 1600; // единица -> миллисекунды
...
function makeConnection(action) {
    // Требуется callback-функция
    let chatSocket = new WebSocket(websocketurl);
    const connect = setInterval(() => {
        // Использование setInterval необходимо, потому что я хочу продолжать
        // подключение к серверу до тех пор, пока сокет не будет открыт.
